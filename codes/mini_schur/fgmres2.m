function [sol,res,its] = fgmres2(A,nD,nG,precfun,rhs,sol,E,F,LD,UD,LG,UG) 
%-----------------------------------------------------------------------
%
...
%
%-----------------------------------------------------------------------
params   %%%%%%%% defines parameters maxits, tol etc. 
%%%%
 n = size(A,1)    ;
 its = 0    ;
% 
...
%% 
      ...
      i = 0  ;
%
   while (i < im  &  (ro/norm(rhs)  >  tol)  &  its < maxits)
      i=i+1  ;
      its = its + 1  ;
      i1 = i + 1 ; 
      z = feval(precfun, nD, nG, E, F, LD, UD, LG, UG, vv(:,i));
      Z(:,i) = z; 
%%                 modified GS  ;
      vv(1:n,i1) = A*z; 
      for j=1:i
...
%         ;
      ...
      if ((ro/norm(rhs)  <=  tol) | (its >= maxits)) return; end; 
