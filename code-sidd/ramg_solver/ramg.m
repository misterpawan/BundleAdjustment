...
% x          : [out] The solution after vcycle_cnt V-cycle
% vcycle_cnt : [out] Numbers of V-clcyes performed
% rel_res    : [out] Relative residual norm after each V-cycle
	...
	tic;
	[A_list, P_list, max_level] = Vcycle_GenMat(A, direct_n);
	gm_t = toc;
	
	% Repeat V-cycle until convergence
	tic;
	while ((rn > rn_stop) && (vcycle_cnt <= max_vcycle))
		x  = Vcycle(1, A_list, P_list, b, x, direct_n, PR_coef, smoother, pre_steps, pos_steps);
		r  = b - A * x;
		rn = norm(r, 2);
