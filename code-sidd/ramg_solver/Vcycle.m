function x = Vcycle(level, A_list, P_list, b, x0, direct_n, PR_coef, smoother, pre_steps, pos_steps)

% V-cycle - 1. Pre-smoothing 2. Coarse Grid Correction 3. Interpolation 
%           4. Post-smoothing
...
% x         : [out] The new solution after a V-clcye
	...
	% Load coefficient matrix- will be different at each level
	A = cell2mat(A_list(level));
	
	% If the problem is small, solve it directly(direct method)
	n = size(b, 1);
	if (n <= direct_n)
		x = A \ b;
		return;
	...
	% Pre-smoothing - to reduce rough error
	...
	x0  = zeros(coarse_n, 1);
	e_C = Vcycle(level + 1, A_list, P_list, r_C, x0, direct_n, PR_coef, smoother, pre_steps, pos_steps);
