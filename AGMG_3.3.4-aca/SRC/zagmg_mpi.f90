...
!         Gram Schmidt orthogonalization is needed (used within GCR)
    ...
    REAL(kind(0.0d0)), PARAMETER :: K_ItGramS=10.0
...
!
    ...
    INCLUDE 'mpif.h'
    REAL(kind(0.0d0)) :: cputm=0.0d0,eltm=0.0d0,eltmp=0.0d0
...
PRIVATE
PUBLIC :: zagmgpar_setupL1, zagmgpar_relmem,  zagmgpar_smoothsetup
PUBLIC :: zagmgpar_applyprec, zagmgpar_GCR, zagmgpar_FlexCG
PUBLIC :: zagmgpar_setsendrecL1
PUBLIC :: zagmgpar_partroword
...
CONTAINS
  ...
  SUBROUTINE zagmgpar_partroword(n, a, ja, ia, idiag, iext, nzext)
    IMPLICIT NONE
    ...
    COMPLEX(kind(0.0d0)), ALLOCATABLE, TARGET :: w(:)
  ...
  SUBROUTINE zagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    REAL(kind(0.0d0)) :: TOL, BNORM, RESID, dum0, RESID0, FREL, RHO
    COMPLEX(kind(0.0d0)) :: ALPHA, BET0, BET1, BET, dum(4), td
    COMPLEX(kind(0.0d0)), ALLOCATABLE :: SD(:),S(:),fsc(:)
    ...
    IF (init) THEN
       ...
       IF (intmv) THEN
          ...
          ELSE
             CALL zagmgpar_mcsrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%il,x,1,f,-2,flop)
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ...
       IF ( ITER.GT.1 ) THEN
          kk=2
          dum(1) = - ZDOTCM(N, SD(1:n), IONE, S(1+3*N:4*N), IONE)
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_DOUBLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
          BET0=dum(1)
          BET1=BET0/RHO
          CALL ZAXPY(N, BET1, S(1+2*N:3*N), IONE, S(1+3*N:4*N), IONE)
          flop=flop+dble(4*N)
          IF (intmv) THEN
             CALL ZAXPY(N, BET1, SD(1:n), IONE, S(1+4*N:5*N), IONE)
             flop=flop+dble(2*N)
          END IF
       END IF
       CALL ZCOPY(N, S(1+3*N:4*N), IONE, S(1+2*N:3*N), IONE)
       IF (intmv)  THEN
          CALL ZCOPY(N, S(1+4*N:5*N), IONE, SD(1:n), IONE)
       ELSE
          IF (intmv2) THEN
          ...
          END IF
       END IF
       dum(1) =  ZDOTCM(N, S(1+2*N:3*N), IONE, SD(1:n), IONE)
       dum(2) =  ZDOTCM(N,S(1+2*N:3*N),IONE,f,IONE)
       flop=flop+dble(4*N)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:kk),kk,MPI_DOUBLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
       IF (ITER.EQ.1) THEN
          IF (.NOT.init) THEN
             BNORM=SQRT(dble(dum(3)))
       ...
       RHO=dble(dum(1))
       ALPHA=dum(2)/RHO
       IF (RHO.EQ.0.0d0) EXIT
       IF (ITER.EQ.1 .AND. (.NOT.init)) THEN
          CALL ZCOPY(N,S(1+2*N:3*N),IONE,X,IONE)
          CALL ZSCAL(N,ALPHA,X,IONE)
          flop=flop+dble(N)
       ELSE
          CALL ZAXPY(N, ALPHA, S(1+2*N:3*N), IONE, X, IONE)
          flop=flop+dble(2*N)
       END IF
       CALL ZAXPY(N, -ALPHA, SD(1:n), IONE, f, IONE)
       dum0 = DZNRM2(N,f,IONE)**2
       flop=flop+dble(4*N)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum0,1,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
       RESID=dum0
       RESID=SQRT(RESID)
       RESID=RESID/BNORM
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE zagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    COMPLEX(kind(0.0d0)) ::  ALPHA, BET0, tmp2
    REAL(kind(0.0d0)) ::  RESID, RESID2, BNORM2, RESID0
    REAL(kind(0.0d0)) ::  TOL, TOL2BNORM2, TOLT, dum0, t1, t2, rho, tmp1
    COMPLEX(kind(0.0d0)) :: dum(3),xd,t
    COMPLEX(kind(0.0d0)), ALLOCATABLE :: Sc(:),Siv(:),SiR(:)
    COMPLEX(kind(0.0d0)), ALLOCATABLE :: Su(:),R(:),fsc(:)
...
10     CONTINUE
       IF (itm.GT.0)       &
          CALL ZGEMV('C',N,itm,cmplx(1,0,kind(0.0d0)),Sc,N,Sc(1+itm*N:N+itm*N), &
               IONE,cmplx(0,0,kind(0.0d0)),Siv(m+2:m+itm+1),IONE)
       Siv(m+1)=DZNRM2(N, Sc(1+itm*N:N+itm*N), IONE)**2
       Siv(m+itm+2)=ZDOTCM(N, Sc(1+itm*N:(itm+1)*N), IONE, f, IONE)
       flop=flop+dble((4+4*itm)*N+4*itm)
       kk=0
       IF (ITER.EQ.1) THEN
             Siv(m)=dum(3)
             kk=1
       END IF
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,Siv(m+1-kk:m+itm+2),itm+2+kk,MPI_DOUBLE_COMPLEX,   &
                MPI_SUM,ICOMM,ierr)
       IF (ITER.EQ.1) THEN
        IF (.NOT.init) THEN
          BNORM2=dble(Siv(m))
       ...
       t2=0.0d0
       DO i=0,itm-1
          t2=t2+conjg(Siv(m+2+i))*Siv(m+2+i)/SiR(1+i+(i*(i+1))/2)
          Siv(m+2+i)=-Siv(m+2+i)/SiR(1+i+(i*(i+1))/2)
       END DO
       rho=t1-t2
       alpha=Siv(m+itm+2)
       IF (itm.GT.0) THEN
          CALL ZGEMV('N',N,itm,cmplx(1,0,kind(0.0d0)),Sc,N,Siv(m+2:m+itm+1),IONE,    &
                 cmplx(1,0,kind(0.0d0)),Sc(1+itm*N:N+itm*N),IONE)
          IF (scndp) THEN
       ...
       bet0=alpha/rho
       Siv(1+itm)=bet0
       CALL ZAXPY(N, -bet0, Sc(1+itm*N:(itm+1)*N), IONE, f, IONE)
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  RECURSIVE SUBROUTINE zagmgpar_inner_iter(n,X,R,l)
    ...
    dum(1) = ZDOTCM(N,X,IONE,R(1:N,2),IONE)
    dum(2) = ZDOTCM(N,X,IONE,R(1:N,1),IONE)
    dum(3) = DZNRM2(N, R(1:N,1), IONE)**2
    dum(4) = DZNRM2(N, R(1:N,2), IONE)**2
    dum(5) = ZDOTCM(N, R(1:N,1), IONE, R(1:N,2), IONE )
    ...
    IF (ABS(dum(1)).LE.repsmach*ABS(dum(2))) THEN
       ...
       GOTO 110
    END IF
    RESID = BNORM + dble((ABS(bet0)**2)*dum(4))-2*dble(bet0*dum(5))
    IF (RESID.LE.resi*resi*BNORM) THEN
       ...
       RETURN
    END IF
    CALL ZAXPY(N, -bet0, R(1:N,2), IONE, R(1:N,1), IONE)
    ITER = 2
    CALL zagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = ZDOTCM(N,R(1:N,3),IONE,R(1:N,2),IONE)
    dum(2) = ZDOTCM(N,R(1:N,3),IONE,R(1:N,1),IONE)
    dum(3) = ZDOTCM(N,R(1:N,3),IONE,R(1:N,4),IONE)
    IF (.NOT.spd) dum(4) = ZDOTCM(N,X,IONE,R(1:N,4),IONE)
    ...
    CALL ZAXPY(N, zeta, R(1:N,3), IONE, X, IONE)
    kstat(1,l)=MAX(kstat(1,l),iter)
    kstat(2,l)=kstat(2,l)+iter
    flop=flop+dble(21*N)
    IF (.NOT.spd) flop=flop+dble(2*N)
    RETURN
100 CONTINUE
    dum(1)=DZNRM2(N, R(1:N,2), IONE)**2
    dum(2)=ZDOTCM(N, R(1:N,2), IONE, R(1:N,1), IONE )
    dum(3) = DZNRM2(N, R(1:N,1), IONE)**2
...
110 CONTINUE
    ...
    ITER = 2
    CALL zagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = ZDOTCM(N,R(1:N,4),IONE,R(1:N,2),IONE)
    dum(2) = ZDOTCM(N,R(1:N,4),IONE,R(1:N,1),IONE)
    dum(3) = DZNRM2(N,R(1:N,4),IONE)**2
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
...
200 CONTINUE
    dum(1) = DZNRM2(N,R(1:N,2),IONE)**2
    dum(2) = ZDOTCM(N,R(1:N,4),IONE,R(1:N,2),IONE)
    dum(3) = DZNRM2(N,R(1:N,4),IONE)**2
    dum(4) = ZDOTCM(N,R(1:N,2),IONE,R(1:N,1),IONE)
    dum(5) = ZDOTCM(N,R(1:N,4),IONE,R(1:N,1),IONE)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_DOUBLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
     ...
     CALL ZAXPY( N, y2, R(1:N,3), IONE, X, IONE )
    kstat(1,l)=MAX(kstat(1,l),iter)
    kstat(2,l)=kstat(2,l)+iter
    flop=flop+dble(25*N)
    RETURN
...
200 CONTINUE
  ...
  SUBROUTINE zagmgpar_setupL1(n,a,ja,ia)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
    ...
    nglp=ngl
    IF (1.NE.nlev) THEN
       CALL zagmgpar_aggregation(1,n,a,ja,ia,nc)
       CALL zagmgpar_setup(2,nc)
    ELSE
...
958 FORMAT('****','              Weighted complexity:',f9.2)
  ...
  RECURSIVE SUBROUTINE zagmgpar_setup(l,n)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l,n
    ...
    END IF
    IF (l /= nlev) THEN
       CALL zagmgpar_aggregation(l,n,dt(l)%a,dt(l)%ja,dt(l)%ia,nc)
       CALL zagmgpar_setup(l+1,nc)
    ELSE
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(dble(ngl(1))/maxcoarset)
...
935 FORMAT(i3,'*','       Number of local rows:',i12)
  ...
  SUBROUTINE zagmgpar_aggregation(l,n,a,ja,ia,nc)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l,n,nc
    INTEGER :: ja(*),ia(n+1)
...
999 CONTINUE
    ...
    isize=MAX(isize,1)
    ALLOCATE (jan(isize))
    CALL zagmgpar_setextnum(nc,l,jan,nzp)
    IF (nc.EQ.0) THEN
        DEALLOCATE(jan)
        RETURN
    ENDIF
...
901    FORMAT(i3, &
  ...
  SUBROUTINE zagmgpar_setsendrecL1(n,ja,ia,iext,nzext,news,ifl,listrank)
    USE zagmgpar_mem
    IMPLICIT NONE
...
1020 FORMAT(' The matrix rows on rank',i9, 'have external connections'  &
  ...
  SUBROUTINE zagmgpar_setextnum(nc,l,iw,imax)
    USE zagmgpar_mem
    IMPLICIT NONE
...
!!#ifdef
  ...
  SUBROUTINE zagmgpar_setcgext(l,n,a,ja,ia,idiag,iext,nfine             &
                            ,ao,jao,iw,nout)
    USE zagmgpar_mem
    IMPLICIT NONE
    ...
    DO i = 1, n
       ...
       DO k=iext(i),ia(i+1)-1
          jcol=NINT(dble(buffi(ja(k)-nfine)))
          IF (jcol.GT.0) THEN
             jpos = iw(jcol)
             IF (jpos.EQ.0) THEN
                jao(ipos) = jcol
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    REAL(kind(0.0d0)) :: cputmp
    REAL(kind(0.0d0)) :: resid,tmp
    COMPLEX(kind(0.0d0)), POINTER, DIMENSION(:) :: ascaled
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
    ...
    IF (MOD(ijb,10).GE.2) THEN
      IF (ijob.LE.201 .AND. MOD(ijob,10).LE.2) THEN
        ...
        CALL zagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,iextL1,nzext)
        CALL zagmgpar_setsendrecL1(n,ja,ia,iextL1,nzext                        &
    ...
    ELSE
      ...
      nza=ia(n+1)-ia(1)
      ALLOCATE(scald(n),ascaled(ia(n+1)-ia(1)))
      CALL zagmgpar_diagscal(n,a,ja,ia,dt(1)%idiag,ascaled)
      CALL zagmgpar_setupL1(n,ascaled,ja,ia)
      DEALLOCATE(ascaled)
      CALL zagmgpar_restorelstrank(ilstinL1,listrank,ifirstlistrank)
      ifirstlistrank=n+1
   ...
   !              WRITE(iout,996) IRANK,cputmp
    ...
    IF (wfo) THEN
       ...
       IF (RESID.GT.0 .AND. iter.NE.0) THEN
         tmp=(flop*log(1.0d0/10)/log(RESID))
         WRITE(iout,952) IRANK,tmp/dble(2*nza)
       END IF
   !CPU_TIME: next line may be uncommented if implemented
   !          (see subroutine mestime)
