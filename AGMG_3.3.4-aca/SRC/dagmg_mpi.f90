...
PRIVATE
PUBLIC :: dagmgpar_setupL1, dagmgpar_relmem,  dagmgpar_smoothsetup
PUBLIC :: dagmgpar_applyprec, dagmgpar_GCR, dagmgpar_FlexCG
PUBLIC :: dagmgpar_setsendrecL1
PUBLIC :: dagmgpar_partroword
...
CONTAINS
  ...
  SUBROUTINE dagmgpar_partroword(n, a, ja, ia, idiag, iext, nzext)
    IMPLICIT NONE
    ...
    REAL(kind(0.0d0)) :: a(*), p
    REAL(kind(0.0d0)), ALLOCATABLE, TARGET :: w(:)
  ...
  SUBROUTINE dagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    REAL(kind(0.0d0)) :: ALPHA, BET0, BET1, BET, dum(4), td
    REAL(kind(0.0d0)), ALLOCATABLE :: SD(:),S(:),fsc(:)
    ...
    IF (init) THEN
       IF (intmv) THEN
          ...
          ELSE
             CALL dagmgpar_mcsrmv(n,dt(1)%a,dt(1)%ja,n+1,dt(1)%il,x,1,f,-2,flop)
    ...
    DO WHILE ( ITER.LT.MAXIT .AND. RESID.GT.TOL )
       ...
       IF ( ITER.GT.1 ) THEN
          kk=2
          dum(1) = - DDOT(N, SD(1:n), IONE, S(1+3*N:4*N), IONE)
          CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1),1,MPI_DOUBLE_PRECISION,  &
               MPI_SUM,ICOMM,ierr)
          BET0=dum(1)
          BET1=BET0/RHO
          CALL DAXPY(N, BET1, S(1+2*N:3*N), IONE, S(1+3*N:4*N), IONE)
          flop=flop+dble(4*N)
          IF (intmv) THEN
             CALL DAXPY(N, BET1, SD(1:n), IONE, S(1+4*N:5*N), IONE)
             flop=flop+dble(2*N)
          END IF
       END IF
       CALL DCOPY(N, S(1+3*N:4*N), IONE, S(1+2*N:3*N), IONE)
       IF (intmv)  THEN
          CALL DCOPY(N, S(1+4*N:5*N), IONE, SD(1:n), IONE)
       ELSE
          IF (intmv2) THEN
          ...
          END IF
       END IF
       dum(1) =  DDOT(N, S(1+2*N:3*N), IONE, SD(1:n), IONE)
       dum(2) =  DDOT(N,S(1+2*N:3*N),IONE,f,IONE)
       flop=flop+dble(4*N)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:kk),kk,MPI_DOUBLE_PRECISION,  &
               MPI_SUM,ICOMM,ierr)
       IF (ITER.EQ.1) THEN
          IF (.NOT.init) THEN
             BNORM=SQRT(dum(3))
       ...
       IF (ITER.EQ.1 .AND. (.NOT.init)) THEN
          CALL DCOPY(N,S(1+2*N:3*N),IONE,X,IONE)
          CALL DSCAL(N,ALPHA,X,IONE)
          flop=flop+dble(N)
       ELSE
          CALL DAXPY(N, ALPHA, S(1+2*N:3*N), IONE, X, IONE)
          flop=flop+dble(2*N)
       END IF
       CALL DAXPY(N, -ALPHA, SD(1:n), IONE, f, IONE)
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  SUBROUTINE dagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,ijb)
    ...
    REAL(kind(0.0d0)) ::  ALPHA, BET0, tmp2
    ...
    REAL(kind(0.0d0)), ALLOCATABLE :: Su(:),R(:),fsc(:)
...
10     CONTINUE
       IF (itm.GT.0)       &
          CALL DGEMV('C',N,itm,1.0d0,Sc,N,Sc(1+itm*N:N+itm*N), &
               IONE,0.0d0,Siv(m+2:m+itm+1),IONE)
       Siv(m+1)=DNRM2(N, Sc(1+itm*N:N+itm*N), IONE)**2
       Siv(m+itm+2)=DDOT(N, Sc(1+itm*N:(itm+1)*N), IONE, f, IONE)
       ...
       IF (ITER.EQ.1) THEN
             ...
             kk=1
       END IF
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,Siv(m+1-kk:m+itm+2),itm+2+kk,MPI_DOUBLE_PRECISION,   &
                MPI_SUM,ICOMM,ierr)
       IF (ITER.EQ.1) THEN
        IF (.NOT.init) THEN
          BNORM2=Siv(m)
       ...
       alpha=Siv(m+itm+2)
       IF (itm.GT.0) THEN
          CALL DGEMV('N',N,itm,1.0d0,Sc,N,Siv(m+2:m+itm+1),IONE,    &
                 1.0d0,Sc(1+itm*N:N+itm*N),IONE)
          IF (scndp) THEN
       ...
       bet0=alpha/rho
       Siv(1+itm)=bet0
       CALL DAXPY(N, -bet0, Sc(1+itm*N:(itm+1)*N), IONE, f, IONE)
...
999 FORMAT('****     set x equal to the zero vector and exit')
  ...
  RECURSIVE SUBROUTINE dagmgpar_inner_iter(n,X,R,l)
    ...
    dum(1) = DDOT(N,X,IONE,R(1:N,2),IONE)
    dum(2) = DDOT(N,X,IONE,R(1:N,1),IONE)
    dum(3) = DNRM2(N, R(1:N,1), IONE)**2
    dum(4) = DNRM2(N, R(1:N,2), IONE)**2
    dum(5) = DDOT(N, R(1:N,1), IONE, R(1:N,2), IONE )
    ...
    IF (RESID.LE.resi*resi*BNORM) THEN
       ...
       RETURN
    END IF
    CALL DAXPY(N, -bet0, R(1:N,2), IONE, R(1:N,1), IONE)
    ITER = 2
    CALL dagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = DDOT(N,R(1:N,3),IONE,R(1:N,2),IONE)
    dum(2) = DDOT(N,R(1:N,3),IONE,R(1:N,1),IONE)
    dum(3) = DDOT(N,R(1:N,3),IONE,R(1:N,4),IONE)
    IF (.NOT.spd) dum(4) = DDOT(N,X,IONE,R(1:N,4),IONE)
    ...
    CALL DAXPY(N, zeta, R(1:N,3), IONE, X, IONE)
    kstat(1,l)=MAX(kstat(1,l),iter)
    kstat(2,l)=kstat(2,l)+iter
    flop=flop+dble(21*N)
    IF (.NOT.spd) flop=flop+dble(2*N)
    RETURN
100 CONTINUE
    dum(1)=DNRM2(N, R(1:N,2), IONE)**2
    dum(2)=DDOT(N, R(1:N,2), IONE, R(1:N,1), IONE )
    dum(3) = DNRM2(N, R(1:N,1), IONE)**2
...
110 CONTINUE
    ...
    ITER = 2
    CALL dagmgpar_prec_matv(l,R,R(1,3),R(1,4),R(1,5),.TRUE.)
    dum(1) = DDOT(N,R(1:N,4),IONE,R(1:N,2),IONE)
    dum(2) = DDOT(N,R(1:N,4),IONE,R(1:N,1),IONE)
    dum(3) = DNRM2(N,R(1:N,4),IONE)**2
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:3),3,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
...
200 CONTINUE
    dum(1) = DNRM2(N,R(1:N,2),IONE)**2
    dum(2) = DDOT(N,R(1:N,4),IONE,R(1:N,2),IONE)
    dum(3) = DNRM2(N,R(1:N,4),IONE)**2
    dum(4) = DDOT(N,R(1:N,2),IONE,R(1:N,1),IONE)
    dum(5) = DDOT(N,R(1:N,4),IONE,R(1:N,1),IONE)
       CALL MPI_ALLREDUCE(MPI_IN_PLACE,dum(1:5),5,MPI_DOUBLE_PRECISION,  &
            MPI_SUM,ICOMM,ierr)
     ...
     CALL DAXPY( N, y2, R(1:N,3), IONE, X, IONE )
    kstat(1,l)=MAX(kstat(1,l),iter)
    kstat(2,l)=kstat(2,l)+iter
    flop=flop+dble(25*N)
    RETURN
...
200 CONTINUE
  ...
  SUBROUTINE dagmgpar_setupL1(n,a,ja,ia)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: n
    INTEGER :: ja(*),ia(n+1)
    ...
    nglp=ngl
    IF (1.NE.nlev) THEN
       CALL dagmgpar_aggregation(1,n,a,ja,ia,nc)
       CALL dagmgpar_setup(2,nc)
    ELSE
...
958 FORMAT('****','              Weighted complexity:',f9.2)
  ...
  RECURSIVE SUBROUTINE dagmgpar_setup(l,n)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l,n
    ...
    END IF
    IF (l /= nlev) THEN
       CALL dagmgpar_aggregation(l,n,dt(l)%a,dt(l)%ja,dt(l)%ia,nc)
       CALL dagmgpar_setup(l+1,nc)
    ELSE
       IF (ngl(1).GT.maxcoarset) wcplex(4)=log10(dble(ngl(1))/maxcoarset)
...
935 FORMAT(i3,'*','       Number of local rows:',i12)
  ...
  SUBROUTINE dagmgpar_aggregation(l,n,a,ja,ia,nc)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l,n,nc
    INTEGER :: ja(*),ia(n+1)
...
999 CONTINUE
    ...
    isize=MAX(isize,1)
    ALLOCATE (jan(isize))
    CALL dagmgpar_setextnum(nc,l,jan,nzp)
    IF (nc.EQ.0) THEN
        DEALLOCATE(jan)
        RETURN
    ENDIF
...
901    FORMAT(i3, &
  ...
  SUBROUTINE dagmgpar_setsendrecL1(n,ja,ia,iext,nzext,news,ifl,listrank)
    USE dagmgpar_mem
    IMPLICIT NONE
...
1020 FORMAT(' The matrix rows on rank',i9, 'have external connections'  &
  ...
  SUBROUTINE dagmgpar_setextnum(nc,l,iw,imax)
    USE dagmgpar_mem
    IMPLICIT NONE
...
!!#ifdef
  ...
  SUBROUTINE dagmgpar_setcgext(l,n,a,ja,ia,idiag,iext,nfine             &
                            ,ao,jao,iw,nout)
    USE dagmgpar_mem
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    REAL(kind(0.0d0)), POINTER, DIMENSION(:) :: ascaled
 ...
 910 FORMAT(i3,'*ENTERING AGMG **********************************',&
    ...
    IF (MOD(ijb,10).GE.2) THEN
      IF (ijob.LE.201 .AND. MOD(ijob,10).LE.2) THEN
        ...
        CALL dagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,iextL1,nzext)
        CALL dagmgpar_setsendrecL1(n,ja,ia,iextL1,nzext                        &
    ...
    ELSE
      ...
      nza=ia(n+1)-ia(1)
      CALL dagmgpar_setupL1(n,a,ja,ia)
      CALL dagmgpar_restorelstrank(ilstinL1,listrank,ifirstlistrank)
      ifirstlistrank=n+1
